...
`[-1, 1]`；神经模型在单日小横截面内优化 `1 - corr(score, target_rank)`。大日期均衡切成
不超过 batch size 的块，任何 batch 都不能混入第二个日期。Early stopping 不看该代理
loss，而是在完整 inner selection 上逐日计算真实 Spearman Rank IC，并最大化日期等权均值。
...
| supervised objective | cross-sectional rank correlation |
