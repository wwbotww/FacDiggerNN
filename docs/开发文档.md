...
| M1 | 标准 Parquet、点时校验、内容寻址数据快照 | 已实现 |
...
2026-07-24 完整采集在衍生工具过滤后有 19,033 个候选和 57,101 个
EOD/dividends/splits 请求。规模会随 EODHD metadata 更新而变化，不能把 `probe` 当作
全量下载。historical 模式在 provider 内强制 `quality.gate.status=passed`，然后生成
...
`holdout_eligibility.eligible`。后者由完整日期/样本数契约、单侧 HAC 显著性、Holm 多重比较、
非重叠稳健性和稳定性门禁共同产生；显式 `--unlock-final-holdout` 只是必要条件，不是绕过
`no_go` 的开关。被拒绝的解封不会改写已冻结 run 的 manifest，也不会创建 holdout 产物。
通过后，runner 使用 freeze 内的 final-refit protocol 新建 `train_end = valid_end` 的 snapshot，
让 scaler 和 official Train 截止最后一个 validation 边界，同时保持 outer validation 为空。
它会校验来源文件哈希没有变化、refit 前后 test 键完全一致，再把 12 个 cell 写入
