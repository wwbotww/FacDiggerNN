...
| 训练 | E0—E3、同日横截面排序目标、Train 内 selection、恢复和 split 访问审计 |
...
强制使用 Float32。`DateGroupedBatchSampler` 将同日证券均衡分块且绝不跨日拼批。训练
loss 只用于优化诊断，checkpoint/early stopping 在完整 inner selection 上按逐日真实
Spearman Rank IC 均值选择。E0 LightGBM 使用日期 group 的 LambdaRank，但采用相同
checkpoint 指标。
...
新排序协议的 E1—E3 checkpoint schema v2 保存：
...
只有 dataset 和完整配置哈希一致时才能恢复，防止错误续跑。旧 Huber schema v1 checkpoint
仍可只读回放，但不能 resume 到排序协议，也不能与新 M6 矩阵混用。
...
5. 目标 RTX 2070 Super / 16 GB 机器已有内存优化与旧协议 pilot，但当前排序、显著性和
   final-refit 协议仍需重新完成完整 M6 资源验收。
