...
后续增量新增平台无关的 CPU 准备和健康审计入口；学校只为它们提供独立 CPU 作业包装，
不把 Slurm、队列、凭据路径或学校额度写进训练器/实验配置。

```bash
# 已配置 EODHD_API_TOKEN 时，先看额度、历史候选和缓存估计（两类列表，重试另计）。
.venv/bin/facdigger data eodhd-plan \
  --config /srv/facdigger/configs/eodhd.yaml \
  --reserve-api-calls 10000 --requests-per-minute 300 \
  --output /srv/facdigger/inputs/download-plan.json

# 人工审核预检后才执行全量下载；原 data ingest 不加参数仍保持原行为。
.venv/bin/facdigger data ingest \
  --config /srv/facdigger/configs/eodhd.yaml \
  --reserve-api-calls 10000 --requests-per-minute 300

# CPU-only：data extra 足够，不导入 torch；runtime 可省略。
.venv/bin/facdigger research transformer-prepare \
  --config /srv/facdigger/configs/research.yaml \
  --output /srv/facdigger/inputs/prepared \
  --runtime /srv/facdigger/configs/runtime.yaml

# 训练改用上一步生成的 runtime.yaml，完成矩阵后独立审计。
.venv/bin/facdigger research transformer-audit \
  --run-dir /srv/facdigger/runs/comparison-01 \
  --output /srv/facdigger/reviews/comparison-01-health.json
```

`transformer-prepare` 和 runner 共享唯一 fold 构建/验证实现。输出 `preparation.json`、
`folds.json`、每 fold 的 checksum 清单及可直接传给训练的 `runtime.yaml`；元数据目录必须在
快照目录外。配置先冻结，每完成一个 fold 持久化进度；重复准备校验已完成文件，未完成的
fold 重建，不声称可恢复 feature 计算中途状态。三个 fold 必须来自相同源文件哈希集合，
发现源数据被重新下载或替换则拒绝混用。准备端和下载端需串行操作同一份 bronze。

账户额度保护是 EODHD provider 的显式选项，不改原配置 schema/哈希或生产调用；不算入
研究协议。HTTP 限速计入免费账户查询，余额检查不等于跨服务器原子预留。正式下载中断依靠
有效缓存复用请求，内存聚合仍重做。学校包装的凭据隔离、数据锁和操作顺序见
[ICF 操作说明](../configs/deployment/icf/README.md)。

健康审计只读取已完成结果并在 run 外写新报告。Rank IC 门禁和 checkpoint 选择保持原有
语义；自动检查依据现有 history 重建后半程成功更新的裁剪计数，恰好 10% 不通过。主 5 日
score std 后半程至少三点才可检查严格单调上升，点数不足输出 `insufficient_history`，
而非默认通过；辅助期限同时展示。仍需人工审核“明显更高波动/梯度失稳”和来源局限，详见
[实验验收口径](Transformer因子质量优化设计.md#12-因子质量验收标准)。
...
此前恢复改造完整 pytest **442 项通过**，见[修复复盘第 58 项](项目关键问题与修复复盘.md)。
加入本轮数据准备/健康审计后，完整覆盖率运行 **471 项通过，覆盖率 82%**，Ruff、离线锁
检查、sdist/wheel 构建、文档/CLI 和脚本检查通过，见同文档第 59 项。未运行真实行情采集、
模型下载、正式训练或 holdout 解锁；未部署或切换生产。原生 Windows 文件锁、多 worker
精确恢复、真实 CUDA FP16、Lustre 跨节点锁和实际 Slurm 抢占链不在本次 CPU 验证结论内。
