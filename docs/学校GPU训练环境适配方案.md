...
后续增量补齐独立 CPU 数据准备：`configure_data.py` 生成隔离路径配置，`submit_data.sh`
依次支持额度预检、显式全量下载、三 fold 预构建；默认仅预检，不申请 GPU、不自动续排。
学校凭据只在计算节点读取，下载额度/限速由 provider 显式开启；训练节点只消费已验证的
快照和外置清单。fold 级准备可重入，并拒绝混用源版本。具体命令见
