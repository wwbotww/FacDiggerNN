...
- historical EODHD 在 provider 内必须通过 `quality.gate`，并将结果转换成版本化的
  `facdigger.standard_parquet` 通用证明；`data validate`、snapshot 和 M6 preflight
  只要求该证明 `status=passed` 并核对标准表哈希，工程模式也不能绕过。
