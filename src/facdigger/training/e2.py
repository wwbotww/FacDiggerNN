"""End-to-end E2 transferred PatchTST training, prediction and evaluation."""
...
from facdigger.training.e2_engine import Initializer, train_e2
...
def _new_run_dir(config: E2ExperimentConfig, dataset_id: str) -> tuple[Path, str, str]:
...
def _resume_run_dir(
    ...
    if not checkpoint.is_file():
        raise FileNotFoundError(f"E2 resume checkpoint not found: {checkpoint}")
    if checkpoint.parent.name != "checkpoints":
        raise ValueError("E2 resume checkpoint must be inside a run's checkpoints directory")
    run_dir = checkpoint.parent.parent
    manifest_path = run_dir / "manifest.json"
    if not manifest_path.is_file():
        raise FileNotFoundError(f"E2 run manifest not found: {manifest_path}")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("status") == "complete":
        raise ValueError("cannot resume an already-complete E2 run")
...
def run_e2(
    ...
    *,
    ...
    initial_manifest = {
        ...
        "model_type": "etth1_transferred_patchtst",
    ...
    try:
        model, training_audit, initialization_audit = train_e2(
            config,
        ...
        _write_json(run_dir / "weight_load_report.json", initialization_audit)
        ...
        manifest = {
            **initial_manifest,
            ...
            "weight_loading": initialization_audit,
        ...
        if last_checkpoint.is_file() and not (run_dir / "weight_load_report.json").is_file():
            import torch

            checkpoint = torch.load(last_checkpoint, map_location="cpu", weights_only=False)
            if "initialization_audit" in checkpoint:
                _write_json(
                    run_dir / "weight_load_report.json",
                    checkpoint["initialization_audit"],
                )
        failed_manifest = {
            **initial_manifest,
            "status": "failed",
            ...
            "recoverable_checkpoint": (
                "checkpoints/last.pt" if last_checkpoint.is_file() else None
            ),
        }
