...
def build_forward_excess_return_labels(
    ...
    panel = (
        ...
        .join(
            bars.select("security_id", "trade_date", "open", "close", "adj_factor"),
        ...
        .with_columns(
            pl.col("trade_date").shift(-execution_lag).over("security_id").alias("label_start"),
            pl.col("trade_date").shift(-horizon).over("security_id").alias("label_end"),
            pl.col("_adj_open").shift(-execution_lag).over("security_id").alias("_entry_value"),
            pl.col("_adj_close").shift(-horizon).over("security_id").alias("_normal_exit"),
        )
    )
