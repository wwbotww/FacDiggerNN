...
from facdigger.data.contracts import table_audit, validate_bars
...
def test_store_is_bounded_and_preserves_listed_day_count(tmp_path) -> None:
    ...
    assert pl.read_parquet(current.root / "bars_daily.parquet")["trade_date"].n_unique() == 20
    ...
    assert bars["trade_date"].n_unique() == 20
    assert universe["trade_date"].unique().to_list() == [date(2026, 7, 30)]
    assert universe["listed_days"].min() > 20
...
def test_adjustment_change_requires_full_hot_history_backfill(tmp_path) -> None:
    days = [date(2026, 7, day) for day in range(1, 30) if date(2026, 7, day).weekday() < 5]
